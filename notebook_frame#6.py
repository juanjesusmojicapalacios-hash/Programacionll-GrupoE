...
def eliminar_paciente():
    ...
    if seleccionado:
        ...
        if messagebox.askyesno("Eliminar Paciente", f"¿Estás seguro de eliminar el paciente '{treeview.item(id_item,'values')[0]}'?"):
...
#funcionde elimnar doctores
def eliminar_doctor():
    seleccionado = treeviewD.selection()
    if seleccionado:
        indice = int(seleccionado[0])
        id_item = seleccionado[0]
        if messagebox.askyesno("Eliminar Doctor", f"¿Estás seguro de eliminar el doctor '{treeviewD.item(id_item,'values')[0]}'?"):
...
btn_eliminarD = tk.Button(btn_frameD, text="Eliminar", bg="red", fg="white",command=eliminar_doctor)
...
#evento al vambiar pestaña 
def al_cambiar_pestanña(event):
    pestaña_acitva = pestañas.index(pestañas.select())
    if pestaña_acitva== 0:
        cargar_desde_archivo_pacientes()
    elif pestaña_acitva == 1:
...
pestañas.bind("<<NotebookTabChanged>>", al_cambiar_pestanña)
#iniciar con pacientes cargadps y doctores
cargar_desde_archivo_pacientes()
cargar_desde_archivo_doctores()
